...
const handleCommand = (command) => {
  ...
  switch (cmd) {
    ...
    case 'cd':
      ...
      if (newPath.startsWith(os.homedir())) {
        currentDir = newPath;
        console.log(`You are currently in ${currentDir}`);
      } else {
    ...
    case 'cat':
      ...
      break;
    case 'add':
      createFile(path.join(currentDir, args[0]));
      break;
    case 'rn':
      renameFile(path.join(currentDir, args[0]), path.join(currentDir, args[1]));
      break;
    case 'cp':
    ...
    case 'hash':
      ...
      break;
    case 'compress':
      compressFile(path.join(currentDir, args[0]), path.join(currentDir, args[1]));
      break;
    case 'decompress':
      decompressFile(path.join(currentDir, args[0]), path.join(currentDir, args[1]));
      break;
    case '.exit':
